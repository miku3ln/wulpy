...
/*  static get parameters() {
  ...
  setDistributionCluster(dataMarkers, map) {
    this.addCluster(map, dataMarkers);
  }
  ...
  addCluster(map, markers) {
    ...
    let resourcePath = "assets/app/cluster/";
    let clusterIcons = [
      {min: 2, max: 100, url: resourcePath + "m1.png", anchor: {x: 16, y: 16}, label: labelOptions},
      {min: 100, max: 1000, url: resourcePath + "m2.png", anchor: {x: 16, y: 16}, label: labelOptions},
      {min: 1000, max: 2000, url: resourcePath + "m3.png", anchor: {x: 24, y: 24}, label: labelOptions},
      {min: 2000, url: resourcePath + "m4.png", anchor: {x: 32, y: 32}, label: labelOptions}
    ];
    ...
    map.addMarkerCluster(options).then((markerCluster: MarkerCluster) => {
      this.initEventsCluster(markerCluster,map);
    });

  }

  initEventsCluster(markerCluster, map) {
    markerCluster.on("marker_click").subscribe((params) => {
      let latLng: LatLng = params[0];
      let marker: Marker = params[1];
      let alertConfig = marker.get("alertConfig");
      if (alertConfig) {
        let target = latLng;
        map.animateCamera({
          target: target,
          ...
          this.alert.showAlertPersonalice(alertConfig);
          /*marker.showInfoWindow();*/
        });
      }
    });
    markerCluster.on("cluster_click").subscribe((cluster: any) => {
    ...
    markerCluster.addEventListener("resolution_changed").subscribe((newResolution) => {
      console.log("new rw", newResolution);
      /*   var center = cluster.getCenter();
         var size = cluster.getSize();
         var markers = cluster.getMarkers();*/
    });
    ...
    return [
      {
        ...
        "phone": "808-484-1488",
        "icon": "assets/app/markers/restaurants.png"
      },
      {
        id: 2,
        "position": {
