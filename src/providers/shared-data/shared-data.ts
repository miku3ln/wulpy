...
import {ConfigDataFirebaseProvider} from "../config-data-firebase/config-data-firebase";
...
export class SharedDataProvider {
  ...
  public customerDataWulpyms: { [k: string]: any } = {};
  ...
  public viewManagement: boolean = false;
  ...
  private fcm: FCM,
    ...
    public dataManagerTemplate: DataManagerTemplateProvider,
    //private fb: Facebook,
    public configDataFirebase: ConfigDataFirebaseProvider,
  ) {
    this.initDataTemplateLocal();
    if (this.viewManagement) {
      this.initDataTemplateServer();
  ...
  login(data, type: string = "firebase") {
    this.customerData = data;
    this.storage.set('customerData', this.customerData);
    this.storage.set('customerDataLoginType', type);
    this.subscribePush();
  }
  ...
  logOut() {
    this.loading.show();
    this.errorMessage = '';
    this.storage.get('customerDataLoginType').then((val) => {
        if (val != null || val != undefined) {
          if (val == "firebase") {
            this.configDataFirebase.signOut().then(result => {
              this.customerData = {};
              this.storage.set('customerData', this.customerData);
              this.storage.set('customerDataLoginType', '');
            });
          }

        }else{
          console.log("customerDataLoginType empty")
        }

      }, error => {
        console.log("error customerDataLoginType");
      }
    );
  ...
  /*---------SERVER--------*/
  initDataTemplateServer() {
    this.dataManagerTemplate.initGettingAllBanners()
      .subscribe(
        result => {
          let dataResult: any;
          dataResult = result;
        ...
        error => {
          ...
          console.log("error initGettingAllBanners ");
        })

    this.dataManagerTemplate.initGetAllPages({language_id: this.config.langId}).subscribe(
      result => {
        let dataResult: any;
        dataResult = result;
        let data = dataResult;
      ...
      error => {
        let errorData = <any>error;
        console.log("error initGetAllPages ");
    ...
    this.dataManagerTemplate.initGeAllProducts(data).subscribe(
      result => {
        let dataResult: any;
        dataResult = result;
        let data = dataResult;
      ...
      error => {
        let errorData = <any>error;
        console.log("error initGeAllProducts TAB1 ");
    ...
    this.dataManagerTemplate.initGeAllProducts(data).subscribe(
      ...
      error => {
        let errorData = <any>error;
        console.log("error getting tab 2 ");
    ...
    this.dataManagerTemplate.initGeAllProducts(data).subscribe(
      ...
      error => {
        let errorData = <any>error;
        console.log("error getting tab 3 ");

      });

    this.dataManagerTemplate.initGetAlCategories({language_id: this.config.langId}).subscribe(
      result => {
        let dataResult: any;
        dataResult = result;
        let data = dataResult;
      ...
      error => {
        let errorData = <any>error;
        console.log("error getting initGetAlCategories ");

      });
  }

  /*----------LOCAL----------*/
  initDataTemplateLocal() {
    //getting recent viewed items from local storage
    this.storage.get('customerData').then((val) => {
      if (val != null || val != undefined) this.customerData = val;
      this.storage.get('customerDataLoginType').then((val) => {
        if (val != null || val != undefined) this.customerData["customerDataLoginType"] = val;
